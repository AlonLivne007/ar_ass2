...
def init_lit_counter(f):
    for clause in f:
        for l in clause:
            lit_counter[abs(l)] = lit_counter.get(l, 0) + 1
    ...
    for l, v in lit_counter.items():
        if l not in m and -l not in m:
            if v > current_max:
                current_max = v
                chosen_lit = l
    return  random.choice([chosen_lit, -chosen_lit]) if chosen_lit is not None else None
